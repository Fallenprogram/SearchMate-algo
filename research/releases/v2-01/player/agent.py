"""SearchMate v1-01: an original, deliberately small Python chess searcher.
...
class _SearchTimeout(Exception):
    """Unwind on a deadline or ply cap, preserving the last completed result."""
...
def _quiescence(
    ...
    """Resolve captures/promotions and check evasions before static evaluation."""
    _check_deadline(deadline)
    terminal = _terminal_score(board, ply)
    if terminal is not None:
        return terminal
    if ply >= _MAX_SEARCH_PLY:
        raise _SearchTimeout

    in_check = board.is_check()
    best_score = -_INFINITY
    if not in_check:
        best_score = _evaluate(board)
        if best_score >= beta:
            return best_score
        alpha = max(alpha, best_score)

    for move in _ordered_moves(board):
        if not in_check and not (board.is_capture(move) or move.promotion is not None):
            continue
        _check_deadline(deadline)
        board.push(move)
        try:
